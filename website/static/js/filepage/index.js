...
var FileViewPage = {
    controller: function(context) {
        ...
        $.extend(self.file.urls, {
            delete: waterbutler.buildDeleteUrl(self.file.path, self.file.provider, self.node.id, {waterbutlerURL: self.node.waterbutlerURL}),
            metadata: waterbutler.buildMetadataUrl(self.file.path, self.file.provider, self.node.id, {waterbutlerURL: self.node.waterbutlerURL}),
            revisions: waterbutler.buildRevisionsUrl(self.file.path, self.file.provider, self.node.id, {waterbutlerURL: self.node.waterbutlerURL}),
            content: waterbutler.buildDownloadUrl(self.file.path, self.file.provider, self.node.id, {waterbutlerURL: self.node.waterbutlerURL, direct: true, mode: 'render'})
        });

        if ($osf.urlParams().branch) {
            var fileWebViewUrl = waterbutler.buildMetadataUrl(self.file.path, self.file.provider, self.node.id, {waterbutlerURL: self.node.waterbutlerURL, branch: $osf.urlParams().branch});
            $.ajax({
                dataType: 'json',
                async: true,
                url: fileWebViewUrl,
            ...
            self.file.urls.content = waterbutler.buildDownloadUrl(self.file.path, self.file.provider, self.node.id, {direct: true, mode: 'render', branch: $osf.urlParams().branch, waterbutlerURL: self.node.waterbutlerURL});
        }
