...
class OsfStorageFileNode(BaseFileNode):
    ...
    def check_in_or_out(self, user, checkout, save=False):
        ...
        if (
                self.is_checked_out and self.checkout != user and permissions.ADMIN not in self.target.get_permissions(
                user)) \
                or permissions.WRITE not in self.target.get_permissions(user):
            raise exceptions.FileNodeCheckedOutError()
